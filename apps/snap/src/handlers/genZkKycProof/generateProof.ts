...
export const generateProof = async (
  ...
  const inputs: any = {
    ...
    humanID: zkCert.getHumanID(params.input.dAppAddress),
  };
  ...
  try {
    const convertedProver = await convertProver(params);
    const { proof, publicSignals } = await groth16.fullProveMemory(
      inputs,
      convertedProver.wasm,
      convertedProver.zkeyHeader,
      convertedProver.zkeySections,
    );
 ...
 */
async function convertProver(params: GenZkKycProofParams) {
  // Somehow we need to convert them to Uint8Array to avoid an error inside snarkjs.

  const wasm = Uint8Array.from(Buffer.from(params.prover.wasm, 'base64'));
  const zkeyHeader = {
    q: BigInt(params.prover.zkeyHeader.q),
    r: BigInt(params.prover.zkeyHeader.r),
    ...
    curve: await getCurveForSnarkJS(params.prover.zkeyHeader.curveName),
  };

  const zkeySections = params.prover.zkeySections.map((section) =>
