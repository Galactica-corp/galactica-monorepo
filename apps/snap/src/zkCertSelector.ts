...
import { RegisteredZkCert } from '@galactica-net/core';
...
export async function selectZkCert(
  ...
  availableCerts: RegisteredZkCert[],
  ...
  if (filteredCerts.length === 1) {
    ...
    while (filteredCerts[indexSelection] === undefined) {
      ...
      if (answer === null) {
        throw new Error('User did not choose a zkCertificate.');
      }
