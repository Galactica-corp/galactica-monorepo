...
import { GenZkKycProofParams } from '@galactica-net/core';
...
async function createCircuitData(
  ...
  const params: GenZkKycProofParams = {
    input,
    // dummy values
    prover: {
      wasm,
...
async function writeCircuitDataToJSON(
  ...
  console.log(
    `curve name: ${JSON.stringify(data.prover.zkeyHeader.curveName)}`,
  );
  ...
  data.prover.zkeyHeader.curveName = data.prover.zkeyHeader.curveName;
  delete data.prover.zkeyHeader.curveName;
