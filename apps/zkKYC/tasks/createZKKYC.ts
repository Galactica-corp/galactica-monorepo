...
import { ZkCertStandard, zkKYCContentFields } from '@galactica-net/core';
...
async function main(args: any, hre: HardhatRuntimeEnvironment) {
  ...
  for (let field of zkKYCContentFields.filter(
  ...
  await tx.wait();
  console.log(
    chalk.green(`Issued the zkKYC certificate ${zkKYC.did} on chain`),
  );
