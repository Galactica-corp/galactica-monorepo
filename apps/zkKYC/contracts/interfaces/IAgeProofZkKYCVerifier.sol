...
interface IAgeProofZkKYCVerifier is IVerifierWrapper{
    function verifyProof(uint[2] memory a, uint[2][2] memory b, uint[2] memory c, uint[] memory input) external view returns (bool);

    function fraudInvestigationInstitutions() external view returns (address[] memory);
    function getAmountFraudInvestigationInstitutions() external view returns (uint);
    ...
    function START_INDEX_INVESTIGATION_INSTITUTIONS() external view returns (uint8);
}
