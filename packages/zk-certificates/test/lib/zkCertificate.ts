...
import { reyZkCertificateContentFields, twitterZkCertificateContentFields, ZkCertStandard, zkKYCContentFields } from '@galactica-net/galactica-types';
...
describe('ZkCertificate', () => {
  ...
  describe('ZkKYC', () => {
    it('should generate zkKYC from example', async () => {
      const processedFields = prepareZkCertificateFields(eddsa, kycExample, ZkCertStandard.ZkKYC);
      ...
      expect(zkKYC.contentHash).to.equal("17914719627421525808158970735216338105805814372033137774126563568134622504748");
    });
  });
  ...
  describe('Twitter', () => {
    it('should generate twitter certificate from example', async () => {
      ...
      expect(cert.contentHash).to.equal("5872577928874538589877684565163333687321666710324086726538479530204757359302");
    });

    it('should handle boolean and 0/1 content the same', async () => {
      let processedFields = prepareZkCertificateFields(eddsa, twitterExample, ZkCertStandard.Twitter);
  ...
  describe('Rey', () => {
    it('should generate rey cert from example', async () => {
      const processedFields = prepareZkCertificateFields(eddsa, reyExample, ZkCertStandard.Rey);
      ...
      expect(zkKYC.contentHash).to.equal("3421856034079648403282443072620678413551125654918205943007940688743589153558");
    });
  });
});
