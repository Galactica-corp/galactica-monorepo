...
contract AirdropGateway is AccessControl, Fallback {
    ...
    function setDistribution(
        ...
        require(
            currentDistribution.clientAddress == address(0),
        ...
        require(
            registrationEndTime < claimStartTime,
            'claim can only start after registration ends'
        );
        require(claimStartTime < claimEndTime, 'invalid claim time');
        currentDistribution = AirdropDistribution(
            requiredSBTs,
    ...
    */
    function deposit(uint amount) external onlyRole(CLIENT_ROLE) {
        require(
            currentDistribution.claimStartTime > block.timestamp,
            'claim has already started'
        );
        require(
            currentDistribution.clientAddress == msg.sender,
            'only client can deposit'
        );
        IERC20(currentDistribution.tokenAddress).transferFrom(
            msg.sender,
            address(this),
            amount
        );
    ...
    function withdrawRemainingToken() external onlyRole(CLIENT_ROLE) {
        require(
            currentDistribution.clientAddress == msg.sender,
            'only client can withdraw'
        );
        require(
            currentDistribution.claimEndTime < block.timestamp,
            'claim has not ended yet'
        );
        uint256 amountLeft = currentDistribution.distributionAmount -
            currentDistribution.amountClaimed;
        IERC20(currentDistribution.tokenAddress).transfer(
            msg.sender,
            amountLeft
        );
    }
    ...
    function register(
        ...
        // check the zk proof
        require(
            currentDistribution.tokenAddress != address(0),
            'distribution is not set'
        );
        require(verifierWrapper.verifyProof(a, b, c, input), 'invalid proof');
        require(
            registeredHumanID[humanID] == false,
            'user has already registered'
        );
        require(
            currentDistribution.registrationStartTime < block.timestamp,
            'registration has not started yet'
        );
        require(
            currentDistribution.registrationEndTime > block.timestamp,
            'registration has ended'
        );
        address[] memory requiredSBTs = currentDistribution.requiredSBTs;
        for (uint i = 0; i < requiredSBTs.length; i++) {
            require(
                IERC721(requiredSBTs[i]).balanceOf(msg.sender) > 0,
                'user does not have required SBT'
            );
        }
    ...
    function claim() external {
        ...
        if (currentDistribution.tokenAmountPerUser == 0) {
            currentDistribution.tokenAmountPerUser =
                currentDistribution.distributionAmount /
                currentDistribution.registeredUserCount;
        }
        IERC20(currentDistribution.tokenAddress).transfer(
            msg.sender,
        ...
        currentDistribution.amountClaimed += currentDistribution
            .tokenAmountPerUser;
