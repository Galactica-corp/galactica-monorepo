...
async function main() {
  ...
  for (let i = 0; i < txsToMeasure.length; i++) {
    ...
    try {
      ...
      if (tx.type === 0) { // Legacy transaction
      ...
      } else if (tx.type === 2) { // EIP-1559 transaction
        ...
        calculatedSize += Math.ceil(tx.nonce.toString(16).length / 2);
        calculatedSize += Math.ceil((tx.maxPriorityFeePerGas || 0).toString(16).length / 2);
        calculatedSize += Math.ceil((tx.maxFeePerGas || 0).toString(16).length / 2);
        calculatedSize += Math.ceil(tx.gasLimit.toString(16).length / 2);
      ...
      console.log(`  ❌ Error processing transaction: ${error instanceof Error ? error.message : String(error)}\n`);
    }
  }
