...
export const simpleToExactAmount = (
  ...
  decimals: number | BN = DEFAULT_DECIMALS
): BN => {
...
export const applyDecimals = (
  inputQuantity: number | string | BN,
...
export const applyRatioMassetToBasset = (input: BN, ratio: BN | string): BN =>
  input.mul(ratioScale).div(ratio);

// How many mAssets is this bAsset worth
export const applyRatio = (
  bAssetQ: BN | string | number,
  ratio: BN | string
): BN => BN.from(bAssetQ).mul(ratio).div(ratioScale);
