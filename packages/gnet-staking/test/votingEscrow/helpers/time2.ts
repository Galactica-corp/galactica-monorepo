...
export const startWeek = (epochSeconds: BigNumberish): BN =>
  BN.from(epochSeconds).div(ONE_WEEK).mul(ONE_WEEK);
//export const startCurrentWeek = async (): Promise<BN> => startWeek(await getTimestamp())

export const weekEpoch = (epochSeconds: BigNumberish): BN =>
  BN.from(epochSeconds).div(ONE_WEEK);
